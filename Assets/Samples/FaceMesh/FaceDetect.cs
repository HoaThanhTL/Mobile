...
namespace TensorFlowLite
{
    public class FaceDetect : BaseVisionTask
    {
        ...
        public enum LookDirection
        {
            ...
            Up,
            Down,
            Unfocused // Assuming 'Unfocused' means not looking directly forward
        }
        ...
        public class Result
        {
            ...
            public LookDirection GetLookDirection(float threshold = 0.05f)
            {
                var eyeCenter = (RightEye + LeftEye) * 0.5f;

                if (IsLookingLeft(threshold)) return LookDirection.Left;
                if (IsLookingRight(threshold)) return LookDirection.Right;
                if (IsLookingUp(threshold)) return LookDirection.Up;
                if (IsLookingDown(threshold)) return LookDirection.Down;

                // Default to forward if no other direction is detected
                return LookDirection.Forward;
            }

            private bool IsLookingLeft(float threshold)
            {
                var eyeCenter = (RightEye + LeftEye) * 0.5f;
                return eyeCenter.x < 0.5f - threshold;
            }

            private bool IsLookingRight(float threshold)
            {
                var eyeCenter = (RightEye + LeftEye) * 0.5f;
                return eyeCenter.x > 0.5f + threshold;
            }

            private bool IsLookingUp(float threshold)
            {
                var eyeCenter = (RightEye + LeftEye) * 0.5f;
                return eyeCenter.y < 0.5f - threshold;
            }

            private bool IsLookingDown(float threshold)
            {
                var eyeCenter = (RightEye + LeftEye) * 0.5f;
                return eyeCenter.y > 0.5f + threshold;
            }

            public bool IsFocused(float threshold = 0.05f)
            {
                // Assuming the user is focused if they're looking forward within a certain threshold
                var direction = GetLookDirection(threshold);
                return direction == LookDirection.Forward;
            }
        }
